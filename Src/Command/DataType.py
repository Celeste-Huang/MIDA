...
class paramType(object):
    ...
    def readCov(self,paramCovFile):
        if paramCovFile=="" or (not os.path.isfile(paramCovFile)):
            print('WARNING: paramCovFile (' + paramCovFile + ') is not a file.')
            raise Exception('1') #return
        try:
    ...
    def mapping(self):
        ...
        for i in range(len(self.simuDirList)):
            ...
            if (not os.path.isfile(self.simuDirList[i])):
                print('WARNING: Can not open simulation output (' + self.simuDirList[i] +')')
                raise Exception('File can not open')
            try:
        ...
        if (not os.path.isfile(self.configFile)):
            print('WARNING: Can not open Configuration File (' + self.configFile+')')
            raise Exception('WARNING: Configuration File ('+self.configFile+') can not open')
        ...
        with open(self.configFile) as f:
            try:
                ...
                print('WARNING: Error in executing the mapping operators in '+self.configFile)
                print(ex)
                raise Exception()
